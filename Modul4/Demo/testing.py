...
#TODO: Program ini untuk melakukan pembuktian di tiap proses aritmatikanya
...
def decorator(transformation_func):
    def wrapper(pointA, *args):
        transformed_points = transformation_func(pointA, *args)
        print(transformed_points," point A : ", pointA, " point B: ", args )
...
def main():
    ...
    result_line = line_equation_of(point(titik_awal_x,titik_awal_y), point(titik_awal_y,titik_awal_z))
    # Compute Translation of line equation
    new_translation = translasi(point(titik_awal_x,titik_awal_y), point(titik_awal_y,titik_awal_z))(nilai_tx, nilai_ty)
